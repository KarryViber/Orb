---
name: infocard
description: 编辑级 HTML/CSS 信息卡（直接 inline 进 Markdown，不包 code fence），36 layouts × 29 styles 模板矩阵 + 内容三维分析（density × structure × mood）+ tone sensing 自动选配色。Use when 需要写知识总结卡 / 数据高亮卡 / 事件公告 / 单话题 content card / 杂志级 typography 输出，或用户说「做个 infocard / 信息卡 / 知识卡 / data highlight」。与 popular-web-designs（整站设计系统）区别：本 skill 是单卡模板。
provenance: user-authored
---

# Infocard Generator

> 来源：`markdown-viewer/skills` (GitHub) — 编辑级 HTML/CSS 信息卡。Orb 工作流：直接 inline HTML 进 Markdown，**不**包 code fence。

## When to Use

- 写知识总结卡 / 数据高亮卡 / 事件公告 / 单话题 content card
- 需要杂志级排版（drop cap / 编辑式 hierarchy / 强 typography）的 Markdown 输出
- 与 `popular-web-designs` 区别：本 skill 走「先内容三维分析后选模板」反向流程，模板矩阵专为 *单卡* 设计；popular-web-designs 是整站设计系统

## 模板与样式

29 个 styles + 36 个 layouts 拆分到同目录的 `styles/` `layouts/` 子目录，按需 read。Family 索引见下方「Style Examples」「Layout Skeletons」段。
