---
name: html-architecture-diagram
description: 分层系统架构图（HTML/CSS 直接 inline 进 Markdown，不出图片不依赖外部渲染器），13 layouts × 12 styles 模板矩阵 + 颜色绑定 layer 语义（User/App/AI/Data/Infra/External）。Use when 需要画分层系统架构、微服务拓扑、技术栈分层、企业 multi-tier 应用，且交付为「文档型」（Lark/Notion/Markdown 直渲），或用户说「画个架构图 / system architecture / layered diagram」。与 drawio/fireworks-tech-graph 区别：本 skill 不出图片不出 SVG，纯 HTML 模板。
provenance: user-authored
---

# Architecture Diagram Generator (HTML/CSS Layered)

> 来源：`markdown-viewer/skills` (GitHub) — 分层架构图（HTML/CSS，非 PlantUML）。Orb 工作流：直接 inline HTML 进 Markdown，**不**包 code fence。

## When to Use

- 画 *分层系统架构图*（User → App → AI/Logic → Data → Infrastructure）
- 微服务拓扑、技术栈分层、企业级 multi-tier 应用
- 需要在 Markdown 里直接嵌入、Lark/Notion/对外文档可直接渲染的架构图
- 与 `drawio`/`fireworks-tech-graph` 区别：本 skill 是 HTML 模板矩阵（13 layouts × 12 styles），不出图片不依赖外部渲染器；适合「文档型」交付，不适合 PPT 单图

## 模板与样式

13 个 layouts + 12 个 styles 拆分到同目录的 `layouts/` `styles/` 子目录，按需 read。颜色绑定 *layer 语义* 而非样式层（同一架构换 style 不动 layer 含义）。
