...
src/
  ...
  egress.js            — Outbound message delivery
...
| Worker → Scheduler | `turn_complete` | `{ type: 'turn_complete', text, toolCount, lastTool, stopReason, channelSemantics, segments, gitDiffSummary? }` | Signals that one Claude turn finished; scheduler can deliver final text while keeping the worker alive for future `inject` messages. `segments` is the ordered list of worker-known text segments for the turn; scheduler records them in the turn ledger before funnel delivery. Scheduler drops successful text when `channelSemantics === 'silent'` and records a receipt. `gitDiffSummary` is included when the worker detected source-tree changes during the turn. |
