...
适用范围：`~/Orb/profiles/.claude/skills/`（system-scope，跨 profile 共享）+ `~/Orb/profiles/<your-profile>/workspace/.claude/skills/`（workspace，单 profile）下的所有 SKILL.md。全局 `~/.claude/skills/` 沿用同标准但仅建议不强制。三层 scope 划分见 § 6。
...
3. **过去 30 天是否被召回过？**（若有日志）
   - 否 → 标记删除候选，观察 30 天再删
   - `pin: true` 的核心 skill 跳过 stale/archive 自动治理；只允许人工审查
...
- **核心保护**：确认为长期必备、但低频召回也不能被自动归档的 workspace skill，在 frontmatter 显式加 `pin: true`

`pin: true` 只用于少数核心 skill。它会让 `skill-promotion-tick` 的 production stale/archive 段跳过该 skill；不要把它当作普通保留标签批量添加。
...
| **System** | `~/Orb/profiles/.claude/skills/` | CLI `--add-dir ~/Orb/profiles`（Orb worker 注入） | **karry + eting 共享** |
