...
| --- | --- | --- |
| `task` | `userText`, `fileContent`, `imagePaths`, `threadTs`, `channel`, `userId`, `platform`, `teamId?`, `threadHistory`, `profile`, `model`, `effort`, `attemptId`, optional `channelSemantics`, `channelMeta`, `fragments?: LabeledFragment[]`, `origin`, `maxTurns`, `mode`, `priorConversation`, `disablePermissionPrompt` | Start a new turn for a thread. `teamId?` carries the Slack workspace/team id (Slack-only) |
...
| `turn_complete` | `text`, `toolCount`, `lastTool`, `stopReason`, `channelSemantics`, optional `gitDiffSummary`, `dailyNotesSummary` | One Claude turn finished; scheduler delivers final text while keeping the worker alive for follow-up `inject`. `text` comes from worker `turnBuffer` (all assistant text blocks in this turn, in order, joined by `\n`); CLI `result.result` is only a last-resort fallback when the buffer is empty. Block-level dedup prevents repeated `result` lines in one turn from emitting duplicates. `gitDiffSummary?` and `dailyNotesSummary?` are routing-only metadata and are not rendered to Slack. |
...
| `inject_failed` | `userText`, optional `injectId`, `attemptId`, `fileContent`, `imagePaths`, `channelMeta`, `fragments?: LabeledFragment[]`, `origin` | Follow-up inject could not reach the live CLI session; scheduler fails forward by replaying through a fresh worker. `channelMeta` and `origin` echo the failed inject so the replay task carries the same provenance |
